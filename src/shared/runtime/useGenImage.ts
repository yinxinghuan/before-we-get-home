      ...
      if (sessionId && !useLegacy && requestedSize) {
        ...
        try {
          const task = await generateImageMedia({
            sessionId,
            requestId,
            // Scene dimensions belong to `size`; the source avatar is only an
            // identity reference. Ordinary edit preserves action and staging
            // more reliably than the avatar-output endpoint.
