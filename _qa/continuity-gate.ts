...
import { beforeWeGetHome as cartridge } from '../src/story/cartridges/beforeWeGetHome'
...
const partiallyGrounded = parseStoryProtocol(`中央车站出口的应急灯还在闪烁。
[choices: "检查中央车站的出口"|"给火星总督送量子电报"|"追赶从未出现的银河快递员"]`, cartridge.locale)
const partialNext = applyParsedScene(initial, partiallyGrounded, cartridge, '查看出口')
assert.deepEqual(partialNext.choices.map((choice) => choice.label), ['检查中央车站的出口'], 'one grounded choice survives without padding or unrelated recovery')
