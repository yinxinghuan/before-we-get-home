...
async function verify(width, height) {
  ...
  await page.route('**/note/telegram/user/get/info/**', (route) => route.fulfill({ contentType: 'application/json', body: JSON.stringify({ data: { name: '归途者', head_url: '' } }) }))
  ...
  const expected = [/十八秒语音/, /旧市场/, /47 降到 43/, /橙色雨衣/]
  for (let index = 0; index < expected.length; index += 1) {
    ...
    if ([0, 3].includes(index)) await page.screenshot({ path: `${outputDir}opening-${index === 0 ? 'arrival' : 'message'}-platform-layout-${width}x${height}.png`, fullPage: true })
...
await browser.close()
console.log(JSON.stringify({ ok: true, pages: 4, choicesAfterFinalPage: 3, viewports: ['320x568', '390x844'] }))
