...
const cartridge: StoryCartridge = {
  ...
  initialFacts: { 'test-pages': 3, 'test-ledger-access': false },
  endingDirector: {
    startRequirements: [{ type: 'fact', id: 'test-pages', equals: 4 }, { type: 'fact', id: 'test-ledger-access', equals: true }],
    capabilities: [
      { id: 'amend', label: 'Amend', meaning: 'Reform the ledger', requires: [{ type: 'fact', id: 'test-ledger-access', equals: true }], mandatoryCosts: ['unproven-remain'] },
...
const parsed = parseStoryProtocol(`[fact: id="test-pages" value="4"]
[fact: id="test-ledger-access" value="true"]
...
assert.equal(ready.facts['test-pages'], 4)
assert.equal(ready.facts['test-ledger-access'], true)
